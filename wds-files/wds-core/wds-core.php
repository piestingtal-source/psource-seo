...
function wds_replace_vars ($string, $args=array()) {
	...
	$replacements = array(
		...
		'%%page%%'		 			=> (get_query_var('paged') != 0) ? 'Page ' . get_query_var('paged') . ' von ' . $wp_query->max_num_pages : '',

		'%%spell_page%%'		 	=> (get_query_var('paged') != 0) ? 'Page '. wds_spell_number(get_query_var('paged')) . ' von ' . wds_spell_number($wp_query->max_num_pages) : '',
...
function save_options_sitewide ($whitelist_options) {
	...
	if ( is_multisite() && WDS_SITEWIDE == true && 'update' == $action && isset( $_POST['option_page'] ) && in_array( $_POST['option_page'], array( 'wds_settings_options', 'wds_autolinks_options', 'wds_onpage_options', 'wds_sitemap_options', 'wds_seomoz_options' ) ) ) { // Added singular
		...
		if ( !isset( $whitelist_options[ $option_page ] ) )
			...
			wp_die( __( 'Fehler: Optionsseite nicht gefunden.' , 'wds') );
		...
		if ( !count( get_settings_errors() ) )
			...
			add_settings_error('general', 'settings_updated', __( 'Einstellungen gespeichert.' , 'wds'), 'updated');
