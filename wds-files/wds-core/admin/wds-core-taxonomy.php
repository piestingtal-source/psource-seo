...
<?php
	...
	function term_additions_form( $term, $taxonomy ) {
		...
		$this->form_row( 'wds_title', __( 'SEO Titel' , 'wds'), __( 'Der SEO-Titel wird auf der Archivseite für diesen Begriff verwendet.' , 'wds'), $tax_meta );

		$this->form_row( 'wds_desc', __( 'SEO Beschreibung' , 'wds'), __( 'Die SEO-Beschreibung wird für die Meta-Beschreibung auf der Archivseite für diesen Begriff verwendet.' , 'wds'), $tax_meta );

		$this->form_row( 'wds_canonical', __( 'Kanonisch' , 'wds'), __( 'Der kanonische Link wird auf der Archivseite für diesen Begriff angezeigt.' , 'wds'), $tax_meta );
		...
		else $this->form_row('wds_noindex', sprintf( __( 'Kein Index für %s' , 'wds'), strtolower( $taxonomy_labels->singular_name ) ), '', $tax_meta, 'checkbox');



		if ($global_nofollow) $this->form_row('wds_override_nofollow', sprintf(__('Follow für %s' , 'wds'), strtolower($taxonomy_labels->singular_name)), '', $tax_meta, 'checkbox');

		else $this->form_row( 'wds_nofollow', sprintf( __( 'Kein Follow für %s' , 'wds'), strtolower( $taxonomy_labels->singular_name ) ), '', $tax_meta, 'checkbox' );
