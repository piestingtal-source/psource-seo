...
class WDS_OnPage {
	...
	function wds_title( $title, $sep = '', $seplocation = '', $postid = '' ) {
		...
		if ( is_front_page() && 'posts' == get_option('show_on_front') ) {
			...
			if (class_exists('PSeCommerce_MS') && 'mp_global_products' == $wp_query->get('pagename')) {

				// Global PSeCommerce products

				if (!empty($wds_options['title-mp_marketplace-base'])) $title = wds_replace_vars($wds_options['title-mp_marketplace-base']);

			} else if (class_exists('PSeCommerce_MS') && in_array($wp_query->get('pagename'), array('mp_global_tags', 'mp_global_categories'))) {

				// Global PSeCommerce Tags / Categories
	...
	function wds_metadesc() {
		...
		if (is_singular()) {
			...
			} else if (class_exists('PSeCommerce_MS') && 'mp_global_products' == $wp_query->get('pagename')) {
